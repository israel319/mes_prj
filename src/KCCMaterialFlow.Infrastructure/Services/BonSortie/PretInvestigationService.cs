...
public class PretInvestigationService : BackgroundService
{
    ...
    private async Task CheckOverdueLoansForInvestigationAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var pret in overdueLoans)
        {
            ...
            {
                _logger.LogError(ex, "Erreur investigation prêt {Id}", pret.Id);
            }
        }

        var criticalLoans = overdueLoans.Where(p => p.JoursRetard >= 14).ToList();
        if (criticalLoans.Count > 0)
    ...
    private static async Task SendInvestigationNotificationAsync(
        IEmailNotificationService emailService, Domain.Entities.Pret pret, int joursRetard, CancellationToken cancellationToken)
    {
    ...
    private static async Task SendCriticalAlertAsync(
        IEmailNotificationService emailService, Domain.Entities.Pret pret, int joursRetard, CancellationToken cancellationToken)
    {
    ...
    private static async Task SendDailySummaryAsync(
        IEmailNotificationService emailService, List<Domain.Entities.Pret> criticalLoans, CancellationToken cancellationToken)
    {
