...
public interface IBonEntreeRepository
{
    ...
    Task<BonEntree> CreateAsync(BonEntree bonEntree, CancellationToken cancellationToken = default);

    Task<BonEntree> UpdateAsync(BonEntree bonEntree, CancellationToken cancellationToken = default);
    ...
    /// <summary>
    /// Supprime toutes les approbations existantes pour un bon (avant recréation lors de la soumission).
    /// </summary>
    Task DeleteApprobationsAsync(int bonId, CancellationToken cancellationToken = default);
...
{
    public IReadOnlyList<BonEntree> Items { get; set; } = [];
