...
public class PretExpirationAlertService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                // Calculer le délai jusqu'à la prochaine exécution (8h00)
    ...
    /// <summary>
    /// Vérifie les prêts expirant et envoie les alertes appropriées
    /// </summary>
    private async Task CheckExpiringLoansAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var days in AlertDays)
        {
            try
            {
                // Récupérer les prêts expirant dans X jours
                ...
                foreach (var pret in prets)
                {
                    try
                    {
                        // Calculer les jours restants (négatif = retard)
                        var joursRestants = (pret.DateRetourPrevue.Date - today).Days;

                        // Envoyer l'alerte avec les vraies données du prêt
    ...
    /// <summary>
    /// Récupère les prêts dont la date de retour est à la date spécifiée
    /// </summary>
    private static async Task<IEnumerable<Pret>> GetPretsExpiringOnDateAsync(
        IBonSortieRepository repository,
        DateTime targetDate,
        CancellationToken cancellationToken)
    {
        // Utiliser la méthode existante avec un delta de 0 jour
        // ou chercher les prêts actifs et filtrer par date
    ...
    /// <summary>
    /// Envoie une alerte email pour un prêt spécifique
    /// </summary>
    private async Task SendPretAlertAsync(
        ...
        CancellationToken cancellationToken)
    {
        // Déterminer le type d'alerte et la couleur
        ...
        // TODO: Récupérer l'email depuis Active Directory ou une table Utilisateurs
        var demandeurEmail = $"{pret.NomDemandeur.ToLower().Replace(" ", ".")}@kcc.cd";

        // En production, récupérer aussi l'email de l'emprunteur et du superviseur
        var recipients = new List<string> { demandeurEmail };

        // Pour les retards importants, ajouter Identification en copie
        string[]? cc = null;
        if (joursRestants <= -3)
        {
